...
public final class MetricsAveragingPublisher implements AutoCloseable {
    ...
    private int httpUpdatesSincePublish = 0;
    ...
    public MetricsAveragingPublisher(SystemMonitor monitor, MqttPublisherConfig config) {
        ...
        this.timeoutSeconds = pollSeconds * Math.max(this.sampleWindow, this.publishEveryHttpUpdates);
    ...
    private void connectMqtt() throws MqttException {
        synchronized (mqttLock) {
            ...
            if (!mqttClient.isConnected()) {
                ...
                LOG.info("[MQTT] Conectado al broker {} con clientId {}", config.brokerUri(), config.clientId());
    ...
    private void pollAndMaybePublishSafely() {
        try {
            ...
            synchronized (window) {
                ...
                httpUpdatesSincePublish++;
                if (snapshot != null && httpUpdatesSincePublish >= publishEveryHttpUpdates) {
                    snapshotToPublish = snapshot;
                    httpUpdatesSincePublish = 0;
                }
            }
            if (snapshotToPublish != null) {
                publish(snapshotToPublish);
                LOG.info("[MQTT] Publicada ventana de {} muestras ({} segundos)", snapshotToPublish.samples(), snapshotToPublish.windowSeconds());
            }
        } catch (Exception ex) {
    ...
    private void publish(AggregatedMetricsSnapshot payload) {
        ...
        try {
            ...
            LOG.info("[MQTT] Publicadas {} muestras en {} (snapshot ts={})",
                    payload.samples(),
                    topic,
                    payload.latestSampleTimestamp());
        } catch (MqttException e) {
    ...
    private void publishAvailability(String status) {
        ...
        try {
            ...
            LOG.info("[MQTT] Estado '{}' publicado en {}", status, topic);
        } catch (MqttException e) {
