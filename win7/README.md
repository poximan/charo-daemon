...
- mqtt.*: parametros del publicador MQTT (URI, plantilla de topico `charodaemon/host/{clientId}/metrics`, ventana de promedio, cadencia de publicacion y client id).
...
- mqtt.publish.every.http.updates: publicar MQTT cada N actualizaciones de `/metrics`.
...
- GET /metrics: snapshot agregado de CPU/MEM (promedio en ventana completa), mas redes y procesos vigilados.
...
- Esta implementacion no expone extension SCADA adapter. No existen endpoints `/scada` ni `/scada/tags` en win7.
